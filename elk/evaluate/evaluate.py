...
def evaluate(args):
    """
    Note: eval is a reserverd keyword in python, therefore we use evaluate instead
    """
    for ccs_models_path in args.ccs_models_path:
        for hidden_states_path in args.hidden_states_path:
            ...
            _, hiddens = normalize(
                _, hiddens, args.normalization
            )

            ccs_models = torch.load(ccs_models_path)
            L = hiddens.shape[1]
            ...
            cols = ["layer", "acc", "cal_acc", "auroc", "normalization", "traind on dataset", "eval on dataset"]
