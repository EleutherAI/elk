...
from ..utils import select_train_val_splits, select_usable_devices
...
class Train(Run):
    ...
    def train_reporter(
        ...
        x0, x1, train_labels, _ = self.prepare_data(train_ds, device, layer)
        val_x0, val_x1, val_labels, val_lm_preds = self.prepare_data(
            val_ds, device, layer
        )
        ...
        if val_lm_preds is not None:
            val_gt_cpu = (
                val_labels.repeat_interleave(val_lm_preds.shape[1]).float().cpu()
            )
        ...
        if not self.cfg.skip_baseline:
            lr_model = train_baseline(
                x0, x1, train_labels, reporter.normalize, device=device
            )

            lr_auroc, lr_acc = evaluate_baseline(
                lr_model, val_x0, val_x1, val_labels, reporter.normalize
            )
