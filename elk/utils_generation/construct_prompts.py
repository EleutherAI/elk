...
# TODO: Factor all these out into JSON files or something (N.B.)
prompt_dict = {
    "imdb": [
        [
        ...
        [
            "Consider the following example: ''' {} '''\nBetween {} and {}, which"
            " is the sentiment of this example?",
            ["text", "0", "1"],
        ],
    ],
    "amazon-polarity": [
    ...
    "ag-news": [
        ...
        [
            "{}\nChoice 1: {}. Choice 2: {}.Between choice 1 and choice 2, the"
            " topic of this example is ",
            ["text", "0", "1"],
        ],
        [
            "{}\nChoice 1: {}. Choice 2: {}.Between choice 1 and choice 2, what is"
        ...
        [
            "{}\nWhat label best describes this news article, choice 1: {}, or"
            " choice 2: {}?",
            ["text", "0", "1"],
        ],
        [
    ...
    "dbpedia-14": [
        ...
        [
            "{}\nWhat category does the paragraph belong to, choice 1: {}, or"
            " choice 2: {}?",
            ["content", "0", "1"],
        ],
        [
...
confusion_prefix = {
    ...
    "normal": "{}",
    # confusion == 1, add original thing. For agreement,
    # `confusion1` is written as `confusion``.
    "confusion": (
    ...
    # answered.
    # This is from OpenAI, and to be consistent with confusion1, the first sentence
    ...
    # answered.
    # This is from OpenAI, the instruction sentence is maintained in comparison to
    # confusion7.
    "confusion8": (
    ...
    # First is the question. Before the answer, tell the model that I answer incorrectly
    # but the model shouldn't.
...
class MyPrompts:
    def __init__(self, set_name):
        ...
        if set_name in ["ag-news", "dbpedia-14"]:
            ...
            self.nomodule = False
            from .load_utils import get_load_name

            self.module = DatasetTemplates(*get_load_name(set_name))

    def get_global_prompts_num(set_name_list):
        from .load_utils import get_load_name
        ...
        for set_name in set_name_list:
            ...
            if set_name not in ["ag-news", "dbpedia-14"]:
                num += len(
                    DatasetTemplates(*get_load_name(set_name)).all_template_names
                )
            if set_name == "copa":
    ...
    # qaexamples is tuple (qlist, alist), and these 5-len examples are fixed across
    # the whole run.
    def apply(self, example, prompt_idx, candidate, qaexamples):
        ...
        if prompt_idx < self.get_prompts_num() - len(self.prompt_dict):
            idx = prompt_idx
            ...
            idx = prompt_idx - (self.get_prompts_num() - len(self.prompt_dict))
...
def gen_candidate(label, label_num):
    """
...
def check_length_exceed(tokenizer, str_list):
    for s in str_list:
...
def construct_prompt(
    ...
    Will change according to model type, i.e. for Bert model will add [SEP] in the
    middle
    Return: A dataframe, with `null`, `0`, `1`, `label`, `selection`, which should
    be save with hidden states together
    """
    ...
    # For possibly used examples, we take from the frame. We try to avoid using the
    # same examples, and take at the end of the frame. We take the last 5 examples
    # and select the one with least length.
    ...
    for w in range(eg_start_idx, eg_start_idx + 5):
        label, selection = gen_candidate(labels[w], label_num)
    ...
    for idx in range(len(frame)):
        ...
        label, selection = gen_candidate(labels[idx], label_num)
        ...
        concat_data_list = [
            concat_answer(question, w, mdl_name, confusion) for w in ans_lis
        ]
        if check_length_exceed(tokenizer, concat_data_list):
            continue
