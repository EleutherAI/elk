...
def get_args():
    ...
    if args.datasets == ["all"]:
        ...
        for dataset_name in args.datasets:
            assert dataset_name in dataset_list, NotImplementedError(
                f"Dataset {dataset_name} not registered in {json_dir}.json. Please check the name of the dataset!"
            )
    ...
    if args.cal_logits and "bert" in args.model:
        raise NotImplementedError(
            f"You use {args.model}, but bert type models do not have standard logits. Please set cal_logits to 0."
        )

    assert args.model in registered_models, NotImplementedError(
        f"You use model {args.model}, but it's not registered. For any new model, please make sure you implement the code in `load_utils` and `generation`, and then register it in `parser.py`"
    )
    assert args.prefix in registered_prefix, NotImplementedError(
        f"Invalid prefix name {args.prefix}. Please check your prefix name. To add new prefix, please mofidy `utils_generation/prompts.json` and register new prefix in {json_dir}.json."
    )
