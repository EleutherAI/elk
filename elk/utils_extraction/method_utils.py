...
print(
    ...
    " `get_dic()`.\n    permutation_dict: Dict of permutation loaded from"
    " `get_dic()`.\n    projection_dict: Key is set_name, each value is a list of"
    " prompt_idx that is used to do projection.\n    test_dict: Test indices, results"
    " in this list will be return.\n    projection_method: The method you use to do"
    " projection. Can be `PCA` or `UMAP`.\n    n_components: The dimension you want to"
    " reduce to. -1 means no projection will be implemented.\n    projection_only:"
    " Default is false. When set to true, will directly return the `projection_model`,"
    " and `res`, `classify_model` will be None.\n    classification_method: Method used"
    " to predict, including LR, TPC and BSS. Default is BSS.\n    print_more: Whether"
    " to print more.\n    learn_dict: A dict to specify the learning parameters for"
    " torch. See class `classify_model` for details.\n## Output = (res,"
    " projection_model, classify_model) ##\n    res: a dict (key, acc_list). Key is the"
    " name of dataset, and acc_list is a list with each accuracy corresponds to one"
    " prompt of set `key`.\n    projection_model & classify_model: the model after"
    " training. Can be used to do any further prediction.\n"
)
...
def main_results(
    ...
    if print_more:
        print("main_results finished, duration: {}s".format(duration))
    return res, lss, projection_model, classify_model


print(
    "------ Func: printAcc ------\n## Input = (input_dict, verbose) ##\n    input_dict:"
    " The dict generated by `main_results`.\n    verbose: Whether to print dataset"
    " level accuracy.\n## Output ##\n    Directly print the accuracy and return the"
