...
invalid_metric_names = st.text().filter(
    lambda x: x not in ["accuracy", "precision", "recall"]
)
...
def test_get_metric_across_layers_with_invalid_input(
    metric_name, reporter_name, reporter_dir, save_csv
):
