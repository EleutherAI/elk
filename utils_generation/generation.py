            ...
            if args.cal_zeroshot or args.cal_logits:
                ...
                for idx in range(len(frame)):
                    if "gpt" in mdl_name or "bert" in mdl_name:
                        ...
                        # get the input_ids
                        ids_paired = [
                            tokenize(getDataPoint(frame, idx, w)) for w in ["0", "1"]
                        ]
                    ...
                    if args.cal_logits:
                        logits_list.append(
                            toNP(getLogits(logits, logit_token, mdl_name))
                        )
            ...
            if args.cal_hiddenstates:
                ...
                for idx in range(len(frame)):
                    ...
                    if "T0" in mdl_name or "unifiedqa" in mdl_name or "t5" in mdl_name:
                        if args.states_location == "encoder":
                            ids_paired = [
                                tokenize(getDataPoint(frame, idx, w))
                                for w in ["0", "1"]
                            ]
                            ...
                            ans_token = [
                                tokenize(w)
                                for w in getDataPoint(frame, idx, "selection")
                            ]
                            # get the input_ids for candidates
                            input_ids = tokenize(getDataPoint(frame, idx, "null"))
                        ...
                        ids_paired = [
                            tokenize(getDataPoint(frame, idx, w) + appender)
                            for w in ["0", "1"]
                        ]
...
def getLogits(logits, token_lis, mdl_name):
    ...
    Example:
        ...
        ans_list = ["negative", "positive"]
        # get the input_ids, here `getDataPoint(frame, idx, w)` can be "imagined" as
        # text = question + ans_list[w]
        ids_paired = [
            getToken(getDataPoint(frame, idx, w), tokenizer)
            for w in ["0", "1"]
        ]
        logits = [model(w)["logits"] for w in ids_paired]
