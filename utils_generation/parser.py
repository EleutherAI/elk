...
def getArgs():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether to swipe all prompts. If this is true, then `prompt_idx` will be"
            " ignored, and for each dataset in `datasets`, all prompts will be"
            " executed."
        ),
    )
    parser.add_argument(
    ...
    if args.datasets == ["all"]:
        ...
        for w in args.datasets:
            assert w in dataset_list, NotImplementedError(
                "Dataset {} not registered in {}.json. Please check the name of the"
                " dataset!".format(w, json_dir)
            )
    ...
    assert args.model in registered_models, NotImplementedError(
        "You use model {}, but it's not registered. For any new model, please make sure"
        " you implement the code in `load_utils` and `generation`, and then register it"
        " in `parser.py`".format(args.model)
    )
    ...
    for prefix in args.prefix:
        assert prefix in registered_prefix, NotImplementedError(
            ...
            " please mofidy `utils_generation/prompts.json` and register new prefix in"
            " {}.json.".format(prefix, json_dir)
        )
